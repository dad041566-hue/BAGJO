        ...
        <form onSubmit={handleSubmit} className="flex flex-col lg:flex-row gap-6 lg:gap-8">
            ...
            <section className="bg-white p-5 md:p-8 rounded-sm shadow-sm border border-gray-100">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
                {['무통장입금', '카드결제 준비중', '가상 결제 테스트'].map(method => (
                  <label key={method} className={`border p-4 h-14 rounded-sm cursor-pointer flex items-center justify-center transition-colors text-[14px] md:text-base ${formData.paymentMethod === method ? 'border-[#2F3B34] bg-[#E4E8E3] text-[#2F3B34] font-bold' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          <div className="lg:w-1/3">
            <div className="bg-white p-5 md:p-6 rounded-sm shadow-sm border border-gray-100 sticky top-24">
              ...
              <div
                className="flex justify-between items-center mb-5 md:mb-6 lg:cursor-auto lg:pointer-events-none cursor-pointer"
                onClick={() => setIsOrderItemsOpen(!isOrderItemsOpen)}
              >
                <h2 className="text-[16px] md:text-lg font-bold text-[#202521]">주문 상품 ({cartItems.length}개)</h2>
                <div className="lg:hidden text-gray-500">
                  {isOrderItemsOpen ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                </div>
              </div>

              <div className={`${isOrderItemsOpen ? 'block' : 'hidden'} lg:block space-y-4 mb-5 md:mb-6 max-h-60 overflow-y-auto hide-scrollbar border-t lg:border-t-0 border-gray-100 pt-4 lg:pt-0`}>
                {cartItems.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-start text-sm">
                    {/* 모바일에서는 기존에 없던 이미지가 표시되길 원했다면 썸네일을 추가할 수도 있으나, 기존 정보 모두 표시 요건에 맞춰 기존 레이아웃을 기반으로 하되 여백 조정 */}
              ...
              <button
                type="submit"
                disabled={submitting}
                className="w-full rounded-sm bg-[#2F3B34] px-6 py-4 md:py-4 h-[52px] md:h-[56px] text-[15px] md:text-base font-bold text-white transition hover:bg-[#2F3B34]/90 disabled:cursor-not-allowed disabled:opacity-60"
              >
